...
est_extract_base_price_and_volume_discount = ->
	small_item_quantity = parseInt($('#user_estimator_small_item_quantity').val(), 10) || 0
	medium_item_quantity = parseInt($('#user_estimator_medium_item_quantity').val(), 10) || 0
	large_item_quantity = parseInt($('#user_estimator_large_item_quantity').val(), 10) || 0
	extra_large_item_quantity = parseInt($('#user_estimator_extra_large_item_quantity').val(), 10) || 0
...
est_update_slider = (slider) ->
	$('#user_estimator_duration_value').text(slider.duration_string())
	$('#user_estimator_duration').val(slider.duration()).change()
...
est_recalculate = ->
	...
	duration = parseInt($('#user_estimator_duration').val(), 10) || 0
...
est_initialize_estimator = (slider) ->
	...
	$('#user_estimator_duration_minus').click ->
		slider.dec()
		false
	$('#user_estimator_duration_plus').click ->
		slider.inc()
		false
...
$ ->
	...
	fix_pricing_boxes_height('#estimator-page') unless estimator_page.hasClass('hidden')
	$('#estimate-now').click ->
		estimator_page.removeClass('hidden')
		fix_pricing_boxes_height('#estimator-page')
	$( window ).resize ->
		fix_pricing_boxes_height('#estimator-page') unless estimator_page.hasClass('hidden')
